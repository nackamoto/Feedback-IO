...
import { sugProps } from "@/components/suggestions/card-tabletplus";
...
export default function SuggestionPage(){
  ...
  const [sortBy, setSortBy] = useState("more");
  ...
  const suggestions = filterBy === 'all' ? datastore: datastore.filter(card => card.category === filterBy);
  ...
  const DesktopSuggestion = () => {
    ...
    const cards = suggestions.map((card:sugProps, i:number) => (
  ...
  const MobileSuggestions = () => {
    ...
    const cards:JSX.Element = datastore.map((card:sugProps, i:number) => (
